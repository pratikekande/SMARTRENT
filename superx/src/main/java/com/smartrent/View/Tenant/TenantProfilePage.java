...
public class TenantProfilePage {
    ...
    private void setupInitialState() {
        ...
        // MODIFIED: Re-added nameField to the binding
    ...
    /**
     * MODIFIED: This method now includes a fallback to fetch the tenant's full name
     * from the 'users' collection if a 'TenantProfile' does not exist.
     */
    private void loadProfileData() {
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            if (documentSnapshot != null && documentSnapshot.exists()) {
                ...
                // Profile doesn't exist, so fetch the name from the 'users' collection
                this.profileExists = false;
                this.currentTenantProfile.setEmail(this.tenantId);
                
                CompletableFuture.supplyAsync(() -> {
                    try {
                        return ds.getSignupData("users", this.tenantId);
                    } catch (Exception e) {
                        e.printStackTrace();
                        return null;
                    }
                }).thenAccept(userSnapshot -> Platform.runLater(() -> {
                    if (userSnapshot != null && userSnapshot.exists()) {
                        String firstName = userSnapshot.getString("firstName");
                        String lastName = userSnapshot.getString("lastName");
                        String fullName = (firstName != null ? firstName : "") + " " + (lastName != null ? lastName : "");
                        this.currentTenantProfile.setName(fullName.trim());
                    }
                    populateFields(this.currentTenantProfile);
                    loadingIndicator.setVisible(false);
                    displayView.setVisible(true);
    ...
    private void populateFields(TenantProfileModel profile) {
        ...
        // MODIFIED: Only the email field is now disabled
    ...
    private Node createDisplayView() {
        ...
        grid.add(createLabel("Address:"), 0, 3);
