...
public class TenantDashboard {
    ...
    /**
     * MODIFIED: This listener now handles both "In Progress" and "Completed" statuses.
     */
    private void setupMaintenanceListener() {
        ...
        this.maintenanceListener = ds.listenForMaintenanceRequestsForTenant(this.tenantId, (snapshots, error) -> {
            if (error != null) {
                System.err.println("Tenant listen failed: " + error);
                return;
            }
            ...
            if (snapshots != null) {
                for (DocumentChange dc : snapshots.getDocumentChanges()) {
                    if (dc.getType() == DocumentChange.Type.MODIFIED) {
                        ...
                        Platform.runLater(() -> {
                            // Remove placeholder if it exists, for any valid update
                            notificationsContainer.getChildren().removeIf(node -> "placeholder-notification".equals(node.getId()));

                            if ("In Progress".equalsIgnoreCase(request.getStatus())) {
                                Node newNotification = createNotificationItem("🛠️", "#FBBF24", 
                                    "Request In Progress", 
                                    "Work has started on your request for '" + request.getTitle() + "'.");
                                notificationsContainer.getChildren().add(0, newNotification);
                            } else if ("Completed".equalsIgnoreCase(request.getStatus())) {
                                Node newNotification = createNotificationItem("✅", "#22C55E", 
                                    "Request Completed", 
                                    "Your request for '" + request.getTitle() + "' is done.");
                                notificationsContainer.getChildren().add(0, newNotification);
                            }
                        });
                    }
    ...
    private VBox createNotificationsBox() {
        ...
        VBox notifBox = new VBox(15, notifTitle, scrollPane);
