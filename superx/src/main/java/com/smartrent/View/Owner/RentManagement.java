...
public class RentManagement {
    ...
    public VBox getView() {
        ...
        Label title = new Label("All Tenant Rent History");
        title.setFont(Font.font("Arial", FontWeight.EXTRA_BOLD, 32));
        title.setTextFill(Color.web("#1f2937"));

        Label subtitle = new Label("Track all tenant rent payments and download receipts");
    ...
    private VBox createPaymentCard(PaymentData payment) {
        ...
        String tenantName = payment.getTenantName();
        String status = "Paid";
        
        // Construct a more detailed description string
        ...
        Label dateLabel = new Label("Date: " + date + "   |   Tenant: " + tenantName);
        ...
        // Add the new location label to the card
    ...
    private void handleDownloadReceipt(PaymentData payment) {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String date = sdf.format(payment.getPaymentDate().toDate());
        fileChooser.setInitialFileName("Receipt-" + payment.getTenantName().replace(" ", "") + "-" + date + ".pdf");
