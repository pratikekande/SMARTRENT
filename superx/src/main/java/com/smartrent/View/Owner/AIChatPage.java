...
import javafx.concurrent.Task; // <-- IMPORT ADDED HERE
...
public class AIChatPage {
    ...
    private static final String API_KEY = ""; // Replace with your actual Gemini API Key
    ...
    private boolean isNameQuery(String text) {
        String lowerCaseText = text.toLowerCase().trim();
        return lowerCaseText.equals("what is your name") ||
               lowerCaseText.equals("what is your name?") ||
               lowerCaseText.contains("your name") ||
               lowerCaseText.equals("who are you") ||
               lowerCaseText.equals("who are you?");
    }
    ...
    private void sendMessageAndGetResponse() {
        ...
        if (isNameQuery(messageText)) {
            addMessage("Hi i am Lily , your personal assistance", false);
        } else {
            showTypingIndicator(true);
            fetchContextData().thenAccept(context -> {
                String personaInstruction = "You are a helpful AI assistant named Lily. ";
                String fullPrompt = personaInstruction + context + "\n\nBased on the context above, answer the following question concisely: " + messageText;
    ...
    /**
     * MODIFIED: This method now includes retry logic with exponential backoff.
     */
        ...
        Task<String> apiTask = new Task<>() {
            ...
            protected String call() throws Exception {
                ...
                for (int i = 0; i < maxRetries; i++) {
                    ...
                    } else if (response.statusCode() == 503) { // Service Unavailable
                        System.out.println("API is overloaded. Retrying in " + (delay / 1000) + " seconds...");
                        Thread.sleep(delay);
                ...
                throw new RuntimeException("API is still overloaded after " + maxRetries + " retries.");
            }
        };

        apiTask.setOnSucceeded(e -> completableFuture.complete(apiTask.getValue()));
        apiTask.setOnFailed(e -> completableFuture.completeExceptionally(apiTask.getException()));
    ...
    private void addInitialGreeting() {
        Platform.runLater(() -> {
            addMessage("Hello! How can I help?", false);
        });
    }
}